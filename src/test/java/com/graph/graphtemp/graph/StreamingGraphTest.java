...
class StreamingGraphTest {
    ...
    private static List<NodeOutput<AgentGraphState>> drain(
            CompiledGraph<AgentGraphState> graph, String input) {
        List<NodeOutput<AgentGraphState>> collected = new ArrayList<>();
        for (NodeOutput<AgentGraphState> output : graph.stream(
    ...
    void emitsTokenChunks() {
        GraphBuilder builder = new GraphBuilder(
                StubChatModel.replying("셋 으로 나뉘는 답"), registry());

        List<NodeOutput<AgentGraphState>> outputs =
        ...
        List<String> chunks = outputs.stream()
                ...
                .map(o -> ((StreamingOutput<AgentGraphState>) o).chunk())
    ...
    void nonStreamingEmitsNoChunks() {
        GraphBuilder builder = new GraphBuilder(StubChatModel.replying("한 방에"), registry());
    ...
    void toolActivityIsVisibleWhileStreaming() {
        AssistantMessage wantsTool = AssistantMessage.builder()
                ...
                .build();
        GraphBuilder builder = new GraphBuilder(
                new StubChatModel(List.of(wantsTool, new AssistantMessage("답은 42"))), registry());

        List<NodeOutput<AgentGraphState>> outputs =
                drain(builder.buildStreaming(reactSpec(List.of("calculator"))), "6*7 은?");

        AgentGraphState finalState = outputs.getLast().state();
